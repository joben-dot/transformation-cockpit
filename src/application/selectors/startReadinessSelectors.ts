...
export function startReadiness(state:DemoState,id:InitiativeId,day:string) {
 ...
 const prioritized=!!profile&&profile.validFrom<=day&&(!profile.validTo||profile.validTo>=day)&&!!assessment&&["ACCEPTED","OVERRIDDEN"].includes(assessment.status)&&!priorityEligibilityBlockers(state,id,profile).length&&currentEffectPotentials(state,id).every(p=>assessment.effectPotentialIds.includes(p.id));
 const blockers=preparation?startBlockers(state,preparation,day):["Samlat beslutspaket behöver sparas och prövas."];
 return {preparation,assessment,prioritized,blockers,ready:!decision&&!!preparation&&!blockers.length,started:!!decision};
}
