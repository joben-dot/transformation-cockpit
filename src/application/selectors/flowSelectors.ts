...
export function caseFlow(state: DemoState, challengeId: ChallengeId, day: string): FlowStep[] {
  ...
  const priority = Object.values(e.priorityAssessments).filter(a => a.initiativeId === initiative.id && a.steeringProfileVersionId === profile?.id).sort((a,b) => b.assessedAt.localeCompare(a.assessedAt))[0];
  ...
  const changes = commitments.filter(c => c.details && !c.details.changeCompletedAt);
  const planned = commitments.flatMap(c => c.details?.measurementDates.map(date => ({c,date})) ?? []);
