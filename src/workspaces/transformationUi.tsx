...
export function Field({label,value,onChange,type="text",required=true}:{label:string;value:string;onChange:(v:string)=>void;type?:string;required?:boolean}){return <label>{label}<input required={required} type={type} step={type==="number"?"any":undefined} value={value} onInput={type==="date"?e=>onChange(e.currentTarget.value):undefined} onChange={e=>onChange(e.target.value)}/></label>;}
