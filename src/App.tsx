...
function WorkspaceApp({ demoState }: { demoState: DemoState }) {
  ...
  const showGuideProcess=()=>{setGuideExpanded(true);requestAnimationFrame(()=>{guideNavigationRef.current?.scrollIntoView?.({block:"start"});guideNavigationRef.current?.focus?.({preventScroll:true});});};
  const header=<><header className="product-header"><StarlightHeader/><div className="header-method-guide"><details className="global-method" open={guideExpanded} onToggle={e=>setGuideExpanded(e.currentTarget.open)}><summary>Processguide · 10 steg, instruktioner och mallar</summary><MethodGuideNavigation stageId={workArea==="templates"?(route.templateStage??"challenge"):undefined} onStageChange={openGuideStage} openCases={showCaseList} navigationRef={guideNavigationRef} detailId={workArea==="templates"?"guide-stage-detail":undefined}/></details></div><nav aria-label="Arbetsytor"><button className="nav-button" aria-current={workArea==="control"?"page":undefined} onClick={()=>setWorkArea("control")}>Kontrollrum</button><button className="nav-button" aria-current={workArea==="cases"&&!showPortfolio?"page":undefined} onClick={()=>{setCaseSection(undefined);setCaseContext({...caseContext,activeId:undefined});setWorkArea("cases");setShowPortfolio(false);}}>Ärenden</button><button className="nav-button" aria-current={workArea==="cases"&&showPortfolio?"page":undefined} onClick={()=>{setWorkArea("cases");setShowPortfolio(true);setPortfolioSection("comparison");}}>Prioritering</button><button className="nav-button" aria-current={workArea==="effects"?"page":undefined} onClick={()=>{if(workArea==="cases"&&!showPortfolio&&caseContext.activeId){const linked=state.entities.challenges[caseContext.activeId]?.relatedInitiativeIds[0];if(!linked){setCaseSection("material");setFeedback("Det här ärendet behöver först ett beredningsinitiativ. Fortsätt med samma underlag nedan.");return;}selectInitiative(linked);}setEffectSection(undefined);setWorkArea("effects");}}>Effekt och beslut</button><button className="nav-button" aria-current={workArea==="templates"?"page":undefined} onClick={()=>setWorkArea("templates")}>Mallar och hjälp</button></nav></header></>;
              ...
              >
                ...
                <label>
                  ...
                  <input
                    ...
                    type="date"
                    value={neededAt}
                    onInput={(event) => setNeededAt(event.currentTarget.value)}
