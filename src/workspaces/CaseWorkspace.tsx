...
export function CaseWorkspace({ state, dispatch, openPortfolio, context, setContext, feedback, day = new Date().toISOString().slice(0,10) }: {
  state: DemoState;
  ...
  if (active && challenge) {
    const businessComplete = Boolean(challenge.businessCase?.purpose && challenge.businessCase.desiredState && challenge.businessCase.scope && challenge.businessCase.alternatives && challenge.businessCase.doNothingConsequence);
    const qualification = initiative ? deriveQualificationStatus(state, initiative.id) : undefined;
    const stepIndex = challenge.nominationStatus === "DRAFT" ? 0 : !businessComplete ? 1 : qualification?.status !== "QUALIFIED" ? 2 : 3;
    ...
    return <section className="case-workspace" aria-label="Ärendevy">
      <button className="back-link" onClick={() => setContext({ ...context, activeId: undefined })}><ArrowLeft size={16}/> Till ärendeöversikten (filter bevaras)</button>
      ...
      <ol className="process-indicator">{process.map((label, index) => <li className={index < stepIndex ? "done" : index === stepIndex ? "current" : "upcoming"} key={label}>{index < stepIndex && <Check size={14}/>}<span>{label}</span>{index >= 4 && <small>Kommande – ej implementerat</small>}</li>)}</ol>
      <p role="status" className="inline-feedback">{feedback}</p>
      <div className="case-columns">
        <article className="card"><p className="eyebrow">REDIGERBART GEMENSAMT UNDERLAG</p><h2>Utmaning och businesscase</h2><p className="template-note">Demonstrationsmall v1 · konfigurerbar struktur · inte verifierad som PPS-komplett</p>
          <form className="material-form" onSubmit={(event) => { event.preventDefault(); saveMaterial(); }}>
  ...
  return <section className="case-workspace" aria-label="Ärendeöversikt"><div className="overview-head"><div><p className="eyebrow">STRATEGISKA ÄRENDEN</p><h1>Vad behöver ledningens uppmärksamhet?</h1><p>Härledda processteg, faktiska hinder och ägare till nästa handling. Arbetslistan är inte en prioriteringsranking.</p></div><button onClick={()=>{setForm({...emptyCase});setShowNew(!showNew)}}><Plus size={17}/> Registrera utmaning</button></div>
    ...
    <div className="case-table" role="table"><div className="case-row case-header" role="row"><span>Ärende</span><span>Steg och hinder</span><span>Nästa åtgärd</span><span>Ansvar och datum</span></div>{filtered.map((item)=><button className="case-row" role="row" key={item.challengeId} onClick={()=>openCase(item.challengeId)}><span><b>{item.caseNumber}</b><strong>{item.title}</strong><small>{item.area}</small></span><span><i>{stepLabels[item.step]}</i><small>{item.obstacle}</small></span><span><strong>{item.nextAction}</strong>{!item.comparable&&<small>Kan inte jämföras: {item.comparisonReason}</small>}</span><span><b>{item.responsible}</b><small>{item.dueDate??"Datum saknas"} {item.overdue&&<em>Försenad</em>}</small></span></button>)}</div><p className="session-note">Demodatum: {day}. Ändringar finns bara i demosessionen.</p></section>;
}
