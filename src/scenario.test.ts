...
describe('deltagarscenario', () => {
  it('behåller den fasta kostnaden när en kommun lämnar och fördelar den på kvarvarande deltagare', () => {
    ...
    const four = calculateParticipationScenario(municipalities.filter(name => name !== 'Kommun D'))
  ...
  it('håller vald kommuns lokala breakdown skild från aggregerad total', () => {
    ...
    const localExample = scenario.localBreakdown.find(item => item.municipality === 'Kommun B')

    expect(localExample).toBeDefined()
    expect(localExample?.releasedHours).toBe(2100)
    expect(localExample?.releasedHours).not.toBe(scenario.releasedHours)
    expect(localExample?.totalCostKsek).not.toBe(scenario.totalCostKsek)
    expect(localExample?.netEffectKsek).not.toBe(scenario.federatedNetKsek)
  })
})
