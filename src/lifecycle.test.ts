...
describe('initiativets sammanhängande livscykel', () => {
  ...
  it('isolerar deltagande per initiativ och påverkar inte identitet eller livscykelmetadata', () => {
    const a = { ...challenge(), id: 'UTM-A', participants: ['Kommun A'] }
    const b = { ...challenge(), id: 'UTM-B', participants: ['Kommun C'] }
    const changedA = updateParticipants(a as Initiative, ['Kommun A', 'Kommun B'])
    expect(changedA.participants).toEqual(['Kommun A', 'Kommun B'])
    expect(b.participants).toEqual(['Kommun C'])
