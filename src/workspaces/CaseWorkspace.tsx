...
const stepLabels = {
  ...
  PRIORITERAD: "Granskat underlag – startkrav återstår",
...
export function CaseWorkspace({ state, dispatch, openPortfolio, context, setContext, feedback, openEffects, initialSection, onSectionChange, day = new Date().toISOString().slice(0,10) }: {
  ...
  const [requirement, setRequirement] = useSessionDraft(`requirement-${context.activeId}`, { missing: "", reason: "", responsible: "", verifier: "", deadline: "" });
  const [answers, setAnswers] = useSessionDraft<Record<string, { summary: string; evidence: string }>>(`qualification-answers-${context.activeId}`, {});
  ...
  if (active && challenge) {
    if (!detail) return <section className="case-workspace" aria-label="Ärendevy">{location}
      ...
      <FlowOverview onOpenDependency={nodeId=>initiative&&openPortfolio(initiative.id,nodeId)} dispatch={dispatch} state={state} challengeId={challenge.id} day={day} onOpen={key=>{
        ...
        else if(["commitments","decision","implementation","measurement","learning"].includes(key)&&openEffects)openEffects(initiative.id,key);
        else setDetail(key);
      }}/>
    </section>;
    ...
    return <section className="case-workspace" aria-label="Ärendevy">{location}
      ...
      <StepGate dispatch={dispatch} state={state} challengeId={challenge.id} day={day} stepKey={detail} onOpen={key=>{if(key==="conditions"&&initiative)openPortfolio(initiative.id);else if(["commitments","decision","implementation","measurement","learning"].includes(key)&&initiative&&openEffects)openEffects(initiative.id,key);else setDetail(key);}}/>
      ...
      <StepGate footer state={state} challengeId={challenge.id} day={day} stepKey={detail} onOpen={key=>{if(key==="conditions"&&initiative)openPortfolio(initiative.id);else if(["commitments","decision","implementation","measurement","learning"].includes(key)&&initiative&&openEffects)openEffects(initiative.id,key);else setDetail(key);}}/>
    </section>;
  }
